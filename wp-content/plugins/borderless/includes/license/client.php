...
if ( ! class_exists( 'BORDERLESS__LICENSE' ) ) {
	class BORDERLESS__LICENSE {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '' ) {
			 ...
			 */
			$this->data_key            = 'borderless_client_' . strtolower( str_ireplace( array( ' ', '_', '&', '?', '-' ), '_', $product_id ) );
			...
			if ( is_admin() ) {
				...
				$this->wc_am_settings_menu_title         = esc_html__( ' License', $this->text_domain );
				$this->wc_am_settings_title              = $this->software_title . esc_html__( ' Key Activation', $this->text_domain );
		...
		public function register_menu() {
			add_submenu_page(
				'borderless.php',
				sprintf( __( '%s', 'borderless' ), $this->wc_am_settings_menu_title ),
				sprintf( __( '%s', 'borderless' ), $this->wc_am_settings_menu_title ),
				'manage_options',
		...
		public function uninstall() {
			 ...
			 * @since 2.5.1
			 *
			 * Filter borderless_client_uninstall_disable
			 * If set to false uninstall() method will be disabled.
			 */
			if ( apply_filters( 'borderless_client_uninstall_disable', true ) ) {
				global $blog_id;
		...
		public function load_settings() {
			...
			// API Key
			add_settings_section( $this->wc_am_api_key_key, esc_html__( 'Key Activation', $this->text_domain ), array(
				$this,
				'wc_am_api_key_text'
			...
			add_settings_field( 'status', esc_html__( 'Key Status', $this->text_domain ), array(
				$this,
			...
			add_settings_section( 'deactivate_button', esc_html__( 'Deactivation', $this->text_domain ), array(
				$this,
				'wc_am_deactivate_text'
			), $this->wc_am_deactivation_tab_key );
			add_settings_field( 'deactivate_button', esc_html__( 'Deactivate Key', $this->text_domain ), array(
				$this,
				...
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( ! empty( $activation_result ) ) {
						...
						if ( isset( $activate_results[ 'data' ][ 'error_code' ] ) && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
							add_settings_error( 'borderless_client_error_text', 'borderless_client_error', esc_attr( "{$activate_results['data']['error']}" ), 'error' );
							update_option( $this->wc_am_activated_key, 'Deactivated' );
						}
					} else {
						add_settings_error( 'not_activated_empty_response_text', 'not_activated_empty_response_error', esc_html__( 'The License Key activation could not be commpleted due to an unknown error possibly on the store server The activation results were empty.', $this->text_domain ), 'updated' );
					}
				...
				if ( ! empty( $deactivation_result ) ) {
					...
					if ( $activate_results[ 'success' ] === true && $activate_results[ 'deactivated' ] === true ) {
						if ( ! empty( $this->wc_am_activated_key ) ) {
							update_option( $this->wc_am_activated_key, 'Deactivated' );
							add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', esc_html__( 'Key deactivated. ', $this->text_domain ) . esc_attr( "{$activate_results['activations_remaining']}." ), 'updated' );
						}
					...
					add_settings_error( 'not_deactivated_empty_response_text', 'not_deactivated_empty_response_error', esc_html__( 'The License Key activation could not be commpleted due to an unknown error possibly on the store server The activation results were empty.', $this->text_domain ), 'updated' );
				}
			}
		...
		public function update_check( $transient ) {
			...
			if ( isset( $response[ 'data' ][ 'error_code' ] ) ) {
				add_settings_error( 'borderless_client_error_text', 'borderless_client_error', "{$response['data']['error']}", 'error' );
			}
